...
function VerifyEmail({ onAuthSuccess }) {
  ...
  const [form, setForm] = useState({ email: prefillEmail, code: '' });
  ...
  const toPlainText = (value, fallback) => {
    ...
    if (React.isValidElement(value)) {
      console.warn('VerifyEmail: React element detected in toPlainText, using fallback');
      return fallback || '';
    }
    // Check for objects that might be React elements
    if (typeof value === 'object') {
      if (value.$$typeof || value.type || value._owner || value._store) {
        console.warn('VerifyEmail: React element-like object detected, using fallback');
        return fallback || '';
      }
  ...
  const handleVerify = async (event) => {
    ...
    try {
      ...
      if (token && user) {
        ...
        setSuccessSafe(message || 'Email verified! Redirecting...');
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setSuccessSafe(message || 'A new code has been sent.');
      try {
        localStorage.setItem('pendingVerificationEmail', form.email);
      } catch (storageErr) {
        console.warn('Unable to persist pending email:', storageErr);
      }
    } catch (e) {
      let errorMessage = 'Failed to resend the code.';
      if (e && typeof e === 'object') {
        const axiosError = e;
        if (axiosError.response?.data) {
          const errorData = axiosError.response.data;
